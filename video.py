            ...
            if 0 <= y < frame.shape[0] and 0 <= x < frame.shape[1] and 0 <= y + h < frame.shape[0] and 0 <= x + w < frame.shape[1]:
                if "EXPOSED" in label and self.should_apply_blur(label):
                    if nsfw_percentage is not None and nsfw_percentage >= 100:
                        frame[y:y + h, x:x + w] = cv2.GaussianBlur(frame[y:y + h, x:x + w], (51, 51), 50)
                    else:
...
class NudeVideoProcessor:
    def __init__(self, video_path, output_folder, task="video", providers=None, video_output_folder="video_output", blur_rule=0.5):
        ...
        if output_folder is None:
            self.output_folder = "output_frames"  # Default folder for frames output
        else:
        ...
        self.ns_risk_data = []  # New: To store insights
    ...
    def process_video(self):
        ...
        with tqdm(total=int(self.cap.get(cv2.CAP_PROP_FRAME_COUNT)), desc="Processing Frames", unit="frames", ncols=100, mininterval=0.5) as pbar:
            while True:
                ...
                if self.task == "frames":
                    ...
                    # New: Store details about NSFW content for insights
                    self.ns_risk_data.append({
                        'frame_number': frame_count,
                        'detections': [det["class"] for det in detections],
                        'nsfw_score': len(detections)
                    })
        ...
        # New: After processing, generate insights
        self.generate_video_insights(frame_count, exposed_count)

        if self.task == "video":
            self.create_video(frame_list, exposed_count, self.blur_rule)

    # New: Generate insights after processing
    def generate_video_insights(self, total_frames, exposed_count):
        if not self.ns_risk_data:
            return

        nsfw_frames = len([frame for frame in self.ns_risk_data if frame['nsfw_score'] > 0])
        nsfw_percentage = (nsfw_frames / total_frames) * 100
        label_distribution = {}

        # Calculate how many times each NSFW label appeared
        for frame_data in self.ns_risk_data:
            for label in frame_data['detections']:
        ...
        insights_report = {
            ...
            'most_frequent_label': max(label_distribution, key=label_distribution.get, default='None')
        }
        ...
        import json
        with open(os.path.join(self.video_output_folder, 'video_insights.json'), 'w') as f:
